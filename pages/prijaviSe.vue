...
export default{
    ...
    methods: {
        prijaviSe(){
            try {
                axios.post("http://localhost:8000/prijaviSe", {
                }).then(res => {
                navigateTo('/');
            });
...
<template>
    <div class="square">
      <div class="inputWrapper">
        ...
        <MailIcon class="mailIcon">  <label for="email" class="label" :class="{ floated: isFloated('email') }">Mail</label>  </MailIcon>
        <input type="text" id="password" name="password" required @focus="floatLabel('password')">
        <PasswordIcon class="passwdIcon">  <label for="password" class="label" :class="{ floated: isFloated('password') }">Lozinka</label>  </PasswordIcon>
        <button type="submit" class="loginButton">Prijavi se</button>
        ...
        <div>
            ...
            <div class="text2">
                <NuxtLink to="/registracija" id="link">Registrirajte se</NuxtLink>
            </div>
            <div class="text3">
                <NuxtLink to="/" id="link">Zaboravili ste lozinku?</NuxtLink>
            </div>
        </div>
    </div>
    </div>
